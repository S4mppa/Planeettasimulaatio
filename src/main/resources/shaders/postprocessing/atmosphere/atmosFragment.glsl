...
const float scatterR = pow(400 / 700f, 4) * 3;
const float scatterG = pow(400 / 530f, 4) * 3;
const float scatterB = pow(400 / 440f, 4) * 3;
...
//Implented using https://www.shadertoy.com/view/wlBXWK
...
in vec3 sunDir)
{
    ...
    blendFactor = smoothstep(0.0, 1.0, blendFactor);
    groundCol = mix(groundCol, rgb * scatteringOpacity + scatteringColour, weatherClarity);
    return mix(groundCol, rgb * scatteringOpacity + scatteringColour, blendFactor);
}
